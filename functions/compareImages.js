...
const comparePngs = async (actual, baseline, diff, config) => {
  return new Promise((resolve) => {
    try {
      ...
      const diffColor = config.settings && config.settings.diffColor ? config.settings.diffColor : [255, 0, 0];
      const diffColorAlt = config.settings && config.settings.diffColorAlt ? config.settings.diffColorAlt : null;
